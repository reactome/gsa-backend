...
get_gene_fc <- function(expression.data, sample.data, design, data_type, analysis.group.1, analysis.group.2) {
    ...
    # This works to a certain extent, still not sure on how it will be used downstream, since df columns have different names.
    res$Identifier <- rownames(res)
    col_order <- c("Identifier", colnames(res)[1:ncol(res)-1])
...
terapadog <- function (esetm = NULL, exp_de = NULL, paired = FALSE,
    ...
    if (paired) {
        ...
        # The issue lies here: seems like grouped_indexes is empty... and exp_de_ordered has two "Block" columns
    ...
    gsScoreFun <- function(G, block) {
        ...
        # (when we use the true, un-permutated "group" labels).
        ...
        if (ite > 1) {
            ...
            # Unpacks grouped_indexes, so to have "group" labels for each individual sample (RNA count and RIBO count)
            # in a vector
