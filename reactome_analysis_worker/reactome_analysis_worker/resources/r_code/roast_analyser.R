...
process <- function(expression.data, sample.data, design, gene.indices, data.type, analysis.group.1, analysis.group.2) {
    ...
    roast.result <- limma::mroast(y = expression.data, index = gene.indices, design = design, contrast = contrasts, nrot = 1999)

    # convert to the required result output
    gsa.result <- roast.result[, c("Direction", "FDR", "PValue", "NGenes")]
